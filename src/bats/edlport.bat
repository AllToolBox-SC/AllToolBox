...
:EDL-start
busybox sleep 1
if %try_times% GTR 120 ECHO %ERROR%连接edl设备超时!%RESET% && pause.exe 重试 && set try_times=1
set /a try_times+=1
for /f %%a in ('lsusb ^| find /c "Qualcomm HS-USB QDLoader 9008"') do (if %%a LSS 1 busybox sleep 1 && goto EDL-start & if %%a GTR 1 ECHO %ERROR%有多个edl设备连接! 请断开其他设备.%RESET% && pause.exe 重试 && goto EDL-start)
::目标设备已经检测到
