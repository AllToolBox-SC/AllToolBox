...
echo.%info%正在恢复相册...
call adbdevice adb
adb push "%sel__folder_path%\DCIM" /storage/emulated/0/
if %errorlevel%==0 (
    echo.%info%恢复成功
) else (
    echo.%error%恢复失败
...
echo.%info%正在恢复相册...
call adbdevice adb
adb push ".\backup\%backupname%\DCIM" /storage/emulated/0/
if %errorlevel%==0 (
    echo.%info%恢复成功
) else (
    echo.%error%恢复失败
...
echo.%info%正在备份相册...
call adbdevice adb
adb pull /storage/emulated/0/DCIM "%sel__folder_path%"
if %errorlevel%==0 (
    echo.%info%备份成功
) else (
    echo.%error%备份失败
...
set backupname=DCIM_%model%_%RANDOM%
mkdir .\backup\%backupname%
adb pull /storage/emulated/0/DCIM ".\backup\%backupname%"
if %errorlevel%==0 (
    echo.%info%备份成功
) else (
    echo.%error%备份失败
...
menu.exe .\menu\backup_9008.json
set /p MENU=<menutmp.txt
...
:9008-backup-run
set backupname=EDL_%innermodel%_%RANDOM%
