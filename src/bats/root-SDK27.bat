...
ECHO.%INFO%宽容selinux
patch_boot.exe | find "Suc" 1>nul 2>nul || ECHO %ERROR%patch_boot.exe无法运行,请尝试安装VC运行库合集&&pause.exe&&exit
ECHO.%INFO%打包boot
...
if "%havesystemui%"=="1" (
  ...
  ECHO.%INFO%安装XTC Patch_SystemUI模块完成
)
ECHO.%INFO%重启手表
run_cmd "adb reboot"
...
if "%isv3%"=="1" (
    if "%havesystemui%"=="1" (
        call instapp.bat .\apks\130510.apk important
    ) else (
        call instapp.bat .\apks\121750.apk important
    )
) else (
    call instapp.bat .\apks\116100.apk important
)
ECHO.%INFO%系统应用安装完成
if exist .\havesystemui.txt set /p havesystemui=<havesystemui.txt >nul
if exist .\smodel.txt set /p smodel=<smodel.txt >nul
...
ECHO.%WARN%请永远不要卸载SystemPlus和XTCPatch，否则手表无法开机
ECHO.%GRAY%-跨越山海 终见曙光-
ECHO.%INFO%提示:当手表进入长续航模式、睡眠模式等禁用模式时，划到最后一页，点击打开应用列表，即可绕过禁用模式
ECHO.%INFO%提示:你可以在/sdcard/hidden_app_list.txt中填写包名以实现隐藏应用
