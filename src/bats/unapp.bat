...
menu.exe .\menu\unapp.json
set /p MENU=<menutmp.txt
if "%MENU%"=="A" exit /b
if "%MENU%"=="a" exit /b
if "%MENU%"=="1" echo.%INFO% 所有应用列表:%RESET% & adb shell pm list packages & pause.exe & goto MAIN_MENU
if "%MENU%"=="2" echo.%INFO% 第三方应用列表:%RESET% & adb shell pm list packages -3 & pause.exe & goto MAIN_MENU
